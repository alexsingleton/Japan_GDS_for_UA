...
It is possible to show the structure of the object using the str() function. <a id="earnings"></a>
...
Not all tabular data are distributed as textfiles, and another very common format is Microsoft Excel format - .xls or xlsx. Unlike .csv files there are no built in function to read these formats, however, other extension packages exist:
...
We will now perform a similar operation on the SP_311 object. The spatialEco package provides the necessary tool to clip point data to polygons, the function is called point.in.poly():
...
You can now see that this has subset the data to the extent of the peninsula area of San Francisco (see previous plot). 
