...
This returned the mean price, plus the length of the supplied string which is the same as the number of rows in the data frame - i.e. nrow(amsterdam). We can now use our function to create means for aggregations using a very helpful package called doBy which we will load now:
...
```{r}
doBy::summaryBy(price ~ property_type, data = amsterdam, FUN = data_description )
```
...
First we will import a spatial data file that relates to historic building locations, imported from http://maps.amsterdam.nl/open_geodata/.
...
historic = readOGR("data/historic_buildings.geojson", verbose = FALSE)
```
...
The buffers are also shown on a map. We use tmap to make a map of these, tmap will be explained in greater detail later in this course.
