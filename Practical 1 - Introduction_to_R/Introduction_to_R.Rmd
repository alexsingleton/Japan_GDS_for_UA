...
R has a slightly different interface when running on either Windows, Mac OS or Linux; however, it is very common for R to be used within an Integrated Development Environment (IDE). The most common of these is R Studio which can be downloaded here: [www.rstudio.com](https://www.rstudio.com/products/rstudio/). R Studio provides a user friendly interface to R, and helps new users by integrating a series of the components of R into a single window interface.
...
The basic R functions can be expanded by installing additional "packages". Each time you want to work with a new package (e.g. ggplot2), you will need to install it to your device using the install.packages() command. Following this, you can just load the package using library() each time you use it afterwards, without the need to install it each time.
...
This should be set to somewhere where all the files for this course are organised. If not you can change the file path using the
setwd() function:

```{r eval=FALSE}
setwd("CHANGE TO FILE PATH")
```
A guide on changing your working directory can be found here: [universeofdatascience.com/](https://universeofdatascience.com/how-to-change-working-directory-in-r/).
