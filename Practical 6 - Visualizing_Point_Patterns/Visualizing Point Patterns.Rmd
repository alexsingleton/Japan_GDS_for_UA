...
As we showed in the previous practical we can map points using tmap and tmap_bubbles(). Let's process our data so we can plot it easily, we are going to be using sf this time instead of rgdal for spatial operations:
...
```
```{r}
#Convert crimes to a spatial format - simple features
crimes_sf <- st_as_sf(crimes, coords = c("Longitude", "Latitude"))
```
