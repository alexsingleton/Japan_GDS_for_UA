---
title: "Data Viz 3) Visualizing Point Patterns"
...
Now we are going to look at mapping some point data. Let's plot the crimes for Chicago - the coord_map function automatically selects the appropriate projection for our map. Check that you have the R package 'mapproj' installed:
...
A similar representation can be produced with hexagons instead of rectangles. To aggregate to hexagons, first install the 'hexbin' R package:
...
Unfortunately, the geom_hex() function only works with Cartesian coordinates, so coord_map() cannot be used in this instance:
