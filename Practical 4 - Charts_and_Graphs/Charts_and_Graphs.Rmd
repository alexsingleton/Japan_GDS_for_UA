...
Base R has functionality that enables the creation of graphics, and although flexible, it is also very common for static graphics to be created using the very popular [ggplot2](http://ggplot2.org/) package. In this practical we will introduce base R functions, ggplot2 and also highlight Plot.ly as a method by which interactive graphics can be created.
...
As we showed in an earlier practical, we can provide a summary of the attributes using the summary() function:
...
As was shown in a previous practical, a mean can be calculated as follows:
